...
export class StockInService {
      ...
      async (prisma) => {
        ...
        if (kanbanData.incoming_order_stock < kanbanData.stock_in_quantity) {
          ...
          throw new ResponseError(
            400,
            "stock in quantity less than incoming order stock"
          );
        }
  ...
  static async get(
    request: SearchStockInRequest
  ): Promise<Pageable<StockInResponse>> {
    const searchRequest = Validation.validate(
      StockInValidation.SEARCH,
      request
    );
  ...
  static async exportExcel(request: SearchStockInRequest): Promise<any> {
    ...
    const templatePath = path.resolve(
      __dirname,
      "../../template_file/StockInExportTemplate.xlsx"
    );
